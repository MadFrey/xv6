...
userinit(void)
{
  ...
  p->sz = PGSIZE;
  kvmcopymappings(p->pagetable, p->shareTable , 0, p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    if(kvmcopymappings(p->pagetable, p->shareTable, sz, n) != 0) {
    ...
    sz = kvmdealloc(p->shareTable, sz, sz + n);
  }
  p->sz = sz;
  return 0;
}
...
fork(void)
{
  ...
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0||
     kvmcopymappings(np->pagetable, np->shareTable, 0, p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
